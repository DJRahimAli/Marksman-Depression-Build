...
if (!holstered) if (place_meeting(x,y,oWall) || place_meeting(x,y,oBulletWall) || place_meeting(x,y,oCollision)) image_alpha = 0.5; else image_alpha = 1;
...
if (global.hascontrol)
{
	...
	if (global.key_primaryattack_held) && (firingdelay < 0) && (!stopshooting)
	{
		...
		with (oPlayer)
		{
			kickbackx = lengthdir_x(1.5, other.image_angle);
